...
case class ChangePostStatusResult(answerGone: Boolean)
...
trait PostsDao {
  ...
  def changePostStatusImpl(postNr: PostNr, pageId: PageId, action: PostStatusAction,
    ...
    var gotUnanswered = false

    // If the answer to a question got deleted, then change question status to unsolved, and reopen it.
